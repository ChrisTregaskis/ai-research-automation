...
_Guide for testing each research topic manually using semantic npm scripts_
...
To view what day relates to what topic, head to `src/config/research-topics.ts` and view `dayOfWeek` field.

```bash
npm run research:mon # Monday
npm run research:tue # Tuesday
npm run research:wed # Wednesday
npm run research:thu # Thursday
npm run research:fri # Friday
```
